...
async function excluirPedido(data) {
  ...
  pedido.pedidos = pedido.pedidos.filter((pd) => pd.id !== parseInt(data));
  if (pedido) {
    await writeFile(fileName, JSON.stringify(pedido));
    //falta decrementar o NextId
  } else {
    throw new Error("Pedido nao encontrado");
  }
}
...
async function consultarPedido(data) {
  ...
  const pedido = pedidos.find((p) => p.id == data);
...
async function maisVendidos() {
  ...
  pedidos = pedidos.filter((p) => p.entregue === true);
  ...
  for (const x in todosSabores) {
    const index = parseInt(x);
    sabores.push({
      sabor: `${todosSabores[index].nome} - ${todosSabores[index].valorTotalGasto}`,
    });
  }
